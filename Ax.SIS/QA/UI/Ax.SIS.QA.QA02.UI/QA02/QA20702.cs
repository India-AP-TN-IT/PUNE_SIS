...
/* 
 * 프로그램명 : QA20702 X-BAR R 관리 결과 등록
 * 설      명 : 
 * 최초작성자 : 
 * 최초작성일 : 
 * 수정  내용 :
...
namespace Ax.SIS.QA.QA02.UI
{
    ...
    /// </summary>
    public partial class QA20702 : AxCommonBaseControl
    {
        //private IQA20702 _WSCOM;
        ...
        private string PAKAGE_NAME = "APG_QA20702";
        ...
        public QA20702()
        {
            ...
            //_WSCOM = ClientFactory.CreateChannel<IQA20702>("QA02", "QA20702.svc", "CustomBinding");
        ...
        protected override void UI_Shown(EventArgs e)
        {
            try
            {
                ...
                this.groupBox3.Text = this.GetLabel("QA20702_MSG1");
                this.groupBox_main.Text = this.GetLabel("QQA20702_MSG2");
                this.groupBox2.Text = this.GetLabel("QA20702_MSG3");
                ...
                this.grd01.AddColumn(false, true, AxFlexGrid.FtextAlign.L, 120, "측정기", "MEAS_INST", "MEAS_INST");
                this.grd01.AddColumn(false, true, AxFlexGrid.FtextAlign.R, 080, "규격", "STANDARD", "STANDARD");
                ...
                this.grd01.AddColumn(false, true, AxFlexGrid.FtextAlign.R, 100, "Gauge R&R", "GATE_RNR");
        ...
        protected override void BtnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string MEAS_DATE    = this.dte02_DD.GetDateText();

                DataSet source = this.GetDataSourceSchema("MEAS_DATE", "SERIAL", "MEAS_SEQ", "MEAS_METER");
                ...
                for (int i = 1; i <= _SAMPLE_CNT; i++)
                {
                    if (_MEAS_UNIT.Equals("IUOKNG"))
                    {
                        ...
                        if (nme.GetValue().ToString().Length > 0)
                            source.Tables[0].Rows.Add(MEAS_DATE, SERIAL, i, nme.GetValue());
                    }
                    else
                    {
                        ...
                        if (nme.GetValue().ToString().Length > 0 && double.Parse(nme.GetValue().ToString()) > 0)
                            source.Tables[0].Rows.Add(MEAS_DATE, SERIAL, i, nme.GetDBValue());
                    }
                }
        ...
        private void dte02_DD_ValueChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                string date = this.dte02_DD.GetDateText();
        ...
        private bool IsSaveValid()
        {
            try
            {
                if (_SERIAL.Length == 0)
                {
                    //MsgBox.Show("저장할 X-Bar R 관리 기준 정보가 선택되지 않았습니다.");
