    ...
    public partial class WM20410R : AxCommonBaseControl
    {
        ...
        protected override void UI_Shown(EventArgs e)
        {
            try
            {
                ...
                this.grd01.AddColumn(false, true, AxFlexGrid.FtextAlign.R, 70, "ROP", "ROP", "ROP");                
                ...
                CellStyle csSHORT_E = grd01.Styles.Add("STATUS_E");

                csSHORT_E.ForeColor = Color.Black;
                csSHORT_E.BackColor = Color.Yellow;

                CellStyle csSHORT_O = grd01.Styles.Add("STATUS_O");
                csSHORT_O.ForeColor = Color.Black;
                csSHORT_O.BackColor = Color.Lime;

                CellStyle csSHORT_S = grd01.Styles.Add("STATUS_S");
                csSHORT_S.ForeColor = Color.White;
                csSHORT_S.BackColor = Color.Red;
        ...
        #region [공통버튼]
        private void DecoGrid(DEV.Utility.Controls.AxFlexGrid grd)
        {
            for(int row =grd.Rows.Fixed;row<grd.Rows.Count;row++)
            {
                string status = grd.GetValue(row, "STATUS").ToString().ToUpper();
                int col = grd.Cols["STATUS"].Index;
                
                if(status == "EXCESS")
                {
                    grd.SetCellStyle(row, col, "STATUS_E");
                }
                else if (status == "SHORTAGE")
                {
                    grd.SetCellStyle(row, col, "STATUS_S");

                }
                else if (status == "OK")
                {
                    grd.SetCellStyle(row, col, "STATUS_O");
                }
            }
        }
