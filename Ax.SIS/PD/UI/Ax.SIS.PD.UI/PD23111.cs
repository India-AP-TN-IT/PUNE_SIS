 ...
 *              날짜          작성자     이슈
 *              ---------------------------------------------------------------------------------------------
 *              2017-09-13    배명희     PD23110 화면 참고 하여 신규 생성
 *
*/
#endregion
using System;
...
namespace Ax.SIS.PD.UI
{
    ...
    /// <b>실사 생산 실적 관리</b>
    /// </summary>
    public partial class PD23111 : AxCommonBaseControl
    {
        //private IPM23110 _WSCOM;
        private AxClientProxy _WSCOM;
        private string PACKAGE_NAME = "APG_PD23111";
        ...
        protected override void UI_Shown(EventArgs e)
        {
            try
            {
                ...
                this.grd01.AddColumn(false, true, AxFlexGrid.FtextAlign.C, 100, "납품처", "CUST_PLANT", "DELI_PART");
                this.grd01.AddColumn(true, true, AxFlexGrid.FtextAlign.R, 100, "실적수량", "RSLT_QTY", "MES_QTY");
                this.grd01.AddColumn(false, true, AxFlexGrid.FtextAlign.R, 100, "실적조정수량", "CHNG_QTY", "MANUAL_QTY");
                ...
                this.SetRequired(this.lbl01_BIZNM2, this.lbl01_LINECD);
                ...
                DataSet source = new DataSet();
                    ...
                    source = this.grd01.GetValue
                     (
                         ...
                         "CORCD", "BIZCD", "LINECD", "DELI_DATE", "PARTNO", "PARTNM", "ALCCD", "JOB_TYPE", "INV_STATUS", "CUST_PLANT", "CHNG_QTY", "USER_ID", "RSLT_QTY"
                     );
        ...
        private void btn01_EXCEL_FORM_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.grd02.Rows.Count > 0)
                {
                    ...
                    this.sfdExcel.FileName = this.GetLabel("PD23111_FORM");
        ...
        private void grd_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            try
            {
                try
                {
                    ...
                    string CUST_PLANT = grd.GetValue(r, "CUST_PLANT").ToString();

                    DataSet source = this.getHistory(CORCD, BIZCD, DELI_DATE, PARTNO, JOB_TYPE, INV_STATUS, CUST_PLANT);
        ...
        private DataSet getHistory(string corcd, string bizcd, string deli_date, string partno,
                                    string job_type, string inv_status, string cust_plant)
        {
            try
            {
                ...
                param.Add("CUST_PLANT", cust_plant);
        ...
        private bool IsSaveValid(DataSet source)
        {
            try
            {
                ...
                for (int i = 0; i < source.Tables[0].Rows.Count; i++)
                {
                    ...
                    if (this.GetByteCount(dr["CUST_PLANT"].ToString()) == 0)
                    {
                        //{0}번째 행에 {1} 정보가 입력되지 않았습니다.
                        MsgCodeBox.ShowFormat("CD00-0091", rIdx, this.grd01.Cols["CUST_PLANT"].Caption);
                        return false;
                    }

                    if (this.GetByteCount(dr["CUST_PLANT"].ToString()) > 2)
                    {
                        //{0}번째 행에 {1} 정보는 {2}Byte 이상 입력할 수 없습니다.
                        MsgCodeBox.ShowFormat("CD00-0092", rIdx, this.grd01.Cols["CUST_PLANT"].Caption, 3);
                        return false;
                    }
        ...
        private void AddRow()
        {
            ...
            this.grd02.SetValue(1, "CUST_PLANT", "1");
