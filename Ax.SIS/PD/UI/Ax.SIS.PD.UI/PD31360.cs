...
namespace Ax.SIS.PD.UI
{
    ...
    /// </summary>
    public partial class PD31360 : AxCommonBaseControl
    {
        ...
        private void PD31360_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void InitGrid()
        {
            try
            {
                ...
                this.grd01.AddColumn(true, true, AxFlexGrid.FtextAlign.R, 130, "LINE", "LINENM", "LINENM");
                this.grd01.AddColumn(true, true, AxFlexGrid.FtextAlign.C, 060, "POS", "INSTALL_POS", "INSTALL_POS");
        ...
        protected override void BtnQuery_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataTable dt = _WSCOM.ExecuteDataSet("MES.PKG_TVOUT_ALL_LINE_PLANRESULT.GET_ALL_PLANRESULT", paramSet).Tables[0];
        ...
        private void InitData(int iCurHour, ref DataTable dt)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                for (int j = 0; j < dt.Columns.Count; j++)
                {
                    ...
                    {
                        string sLineNm = Convert.ToString(dt.Rows[i]["LINENM"]);
            ...
            try
            {
                ...
                for (int i = 0; i < grd01.Rows.Count; i++)
                {
                    for (int j = 1; j < grd01.Cols.Count; j++)
                    {
                        ...
                        if (colName.StartsWith("TOT"))
                        {
                            string sLineNm = Convert.ToString(grd01.Rows[i]["LINENM"]);
                            int colHour = Convert.ToInt16(colName.Replace("TOT_", ""));
                        ...
                        else if (colName.StartsWith("LINENM") || colName.StartsWith("INSTALL_POS"))
